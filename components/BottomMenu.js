...
export default function BottomMenu({ activePage = 'questions' }) {
  ...
  // Обновляем активную страницу при изменении маршрута
  useEffect(() => {
  ...
  const prefetchData = (page) => {
    ...
    } else if (page === 'pair' && user) {
      // Предзагружаем данные пары
      queryClient.prefetchQuery({
        queryKey: ['pair-code'],
        queryFn: () => import('../utils/api').then(mod => mod.fetchPairCode()),
      });
    }
  };
      ...
      <div className="bottom-menu">
        <Link href="/questions" legacyBehavior>
          <a className="menu-link">
            <div 
              className={`menu-item ${active === 'questions' ? 'active' : ''} ${prevActive === 'questions' && animating ? 'prev-active' : ''}`} 
              onMouseEnter={() => prefetchData('questions')}
            >
        ...
        <Link href="/pair" legacyBehavior>
          <a className="menu-link">
            <div 
              className={`menu-item ${active === 'pair' ? 'active' : ''} ${prevActive === 'pair' && animating ? 'prev-active' : ''}`} 
              onMouseEnter={() => prefetchData('pair')}
            >
              <div className="icon-container">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <Link href="/settings" legacyBehavior>
          <a className="menu-link">
            <div 
              className={`menu-item ${active === 'settings' ? 'active' : ''} ${prevActive === 'settings' && animating ? 'prev-active' : ''}`}
            >
              <div className="icon-container">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <style jsx>{`
          .bottom-menu-spacer {
            height: 70px; /* Высота меню + отступы */
            width: 100%;
          }
          ...
          .bottom-menu {
            ...
            touch-action: none; /* Предотвращает свайп на меню */
            pointer-events: auto; /* Обеспечивает взаимодействие с меню */
            will-change: transform; /* Оптимизирует производительность */
